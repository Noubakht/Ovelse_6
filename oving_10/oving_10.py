    ...
    with open(filnavn_1, 'r', encoding='utf-8') as fil:
        ...
        for linje in fil:
            ...
            try:
                ...
                # legger kun til dato(unødvendig?)
                datoer.append(dato_tid.date())
                # legger kun til tid(unødvendig?)
                tider.append(dato_tid.time())
            except ValueError:
...
print(f"Gjennomsnitt:   {gs_temp: .1f} C")
print(f"Maksimum:       {maks_temp: .1f} C     ved tidspunkt: {maks_temp_tid}")
print(f"Minimum:        {min_temp: .1f} C     ved tidspunkt: {min_temp_tid}")
print("--------------------------------------------------------------")
...
plt.plot(x_trykk_abs, y_trykk_abs, label="Absolutt trykk", color='blue',
           linewidth=0.5)
...
plt.plot(x_lufttrykk_MET, y_lufttrykk_MET, label="Absolutt trykk MET",
...
plt.plot(x_temperatur, y_temperatur, label="Temperatur", color='blue',
           linewidth=0.5)

# Plotting av temperaturer fra den korteste fila (grønn graf)
plt.plot(x_temperatur_2, y_temperatur_2, label="Temperatur MET", color='green',
           linewidth=0.5)
plt.legend()
...
plt.plot(x_gjennomsnitt_temp, y_gjennomsnitt_temp,
